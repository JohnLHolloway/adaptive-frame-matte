...
function dashboard() {
  ...
  return `<div class="two"><div class="card"><div class="row spread"><h2>On your Frame</h2><span class="badge">${esc(state.profile || 'day')} profile</span></div>
    ...
    <div>${palette(state.room?.palette)}</div><small>Art Mode: ${esc(state.artmode || 'unknown')}<br>Artwork changed: ${fmt(state.last_artwork_change)}<br>Matte changed: ${fmt(state.last_matte_change)}<br>TV communication: ${fmt(state.last_communication)}</small></div></div></div>
...
function scores() { return `<div class="scroll"><table><thead><tr><th>Matte</th><th>Score</th>${Object.keys(state.settings.weights).map(k=>`<th>${k}</th>`).join('')}</tr></thead><tbody>${state.recommendations.map(r=>`<tr><td>${esc(r.matte.name)}</td><td>${r.score}</td>${Object.keys(state.settings.weights).map(k=>`<td>${r.components[k]}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`; }
function roomCards() {
 return `<div class="grid">${['day','night'].map(p=>{const r=state.rooms[p]; return `<div class="card"><span class="step">${p} profile</span><h2>${r ? r.descriptors.join(' · ') : 'Ready when you are'}</h2>${r ? `<p>${swatch(r.wall_hex)} ${esc(r.wall_hex)} · ${Math.round(r.confidence*100)}% confidence</p><div>${palette(r.palette)}</div><p>Lightness ${r.lightness.toFixed(1)} · Chroma ${r.chroma.toFixed(1)} · Warmth ${r.warmth.toFixed(1)}</p><small>${r.source === 'guided' ? 'Reference-corrected estimate; phone HDR and white balance limit accuracy.' : 'Manual color estimate; not photograph-calibrated.'}</small>` : '<p>Add this profile now, or return when the room has its normal lighting.</p>'}
  ${r?.photo ? `<img class="thumb" src="/media/room/${r.photo}" alt="${p} room photograph"><p>${button('Edit wall mask',`mask-${p}`,'class="secondary"')}</p>` : ''}
  <form data-form="quick" data-profile="${p}"><h3>Quick wall color</h3><div class="row"><input type="color" name="picker" value="${r?.wall_hex || '#d8d0bd'}" aria-label="Wall color picker"><input name="color" value="${r?.wall_hex || '#d8d0bd'}" pattern="#[0-9a-fA-F]{6}" aria-label="Wall HEX color"><button>Save ${p} color</button></div></form>
  <form data-form="photo" data-profile="${p}"><h3>Guided photograph</h3><p>Leave the calibration image displayed. Stand at approximately your normal viewing position and include the entire TV plus a meaningful amount of surrounding wall.</p><input type="file" name="file" accept="image/jpeg,image/png,image/webp" required><p><button>Analyze ${p} photo</button></p></form>
 </div>`;}).join('')}</div>`;
}
function calibrationControls() {
 return `<div class="card"><h2>Guided room calibration</h2><p>Recommended: use our original 4K sRGB reference image to reduce camera and lighting errors. The TV must already be displaying Art Mode. Automation pauses for the entire calibration session.</p><div class="row">${state.calibration ? button('Finish & restore original artwork','calibration-finish') : button('Display calibration image','calibration-start')}<a class="button secondary" href="/calibration-pattern.png" download>Download reference pattern</a></div>${state.calibration ? `<p>Calibration session: ${esc(state.calibration.phase)}. Matte used: ${esc(state.calibration.matte || 'pending')}.</p>` : ''}<p><small>This estimates appearance, not laboratory colorimetry. TV brightness, HDR, exposure, glare and phone processing remain sources of uncertainty.</small></p>
 ${Object.entries(state.owned).filter(([id,v])=>!v.deleted).map(([id])=>`<p>${button('Remove application calibration image','remove-owned',`data-id="${esc(id)}" class="secondary"`)}</p>`).join('')}</div>`;
}
function schedule() {
...
function setup() {
 ...
 <span class="step">4 / Room calibration</span>${calibrationControls()}${roomCards()}${schedule()}<div class="card"><p>Nighttime photography can wait until your normal evening lighting is available.</p>${button('Finish setup · add nighttime calibration later','finish-setup')}</div>`;
}
function artworks() {
...
function renderPage() {
 const renderers = {dashboard, setup, room:()=>calibrationControls()+roomCards()+schedule(), artwork:artworks, mattes, strategy, history, diagnostics, settings};
...
document.addEventListener('submit', async e=>{
 ...
 try {
  switch(f.dataset.form) {
   ...
   case 'strategy': {const weights={}; for(const k of Object.keys(data)) if(k.startsWith('weight_')) {weights[k.slice(7)]=Number(data[k]); delete data[k];} for(const k of ['threshold','cooldown','neutral_preference']) data[k]=Number(data[k]); await api('/api/settings',{...data,weights}); await refresh(); break;}
